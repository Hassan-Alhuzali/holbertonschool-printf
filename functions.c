...
int print_number_long(long n, int flags) {
  ...
  if (num / 10)
    count += print_number_long(num / 10, 0);
  count += _putchar_buffer((num % 10) + '0');
...
int print_number_short(short n, int flags) {
  ...
  if (num / 10)
    count += print_number_short(num / 10, 0);
  count += _putchar_buffer((num % 10) + '0');

  return count;
}

static int print_unsigned_long_recursive(unsigned long n, char *digits,
                                         int base) {
  int count = 0;

  if (n / base)
    count += print_unsigned_long_recursive(n / base, digits, base);
  count += _putchar_buffer(digits[n % base]);
  return count;
}
...
int print_unsigned_long(unsigned long n, char *digits, int base, int flags) {
  ...
  count += print_unsigned_long_recursive(n, digits, base);
  return count;
}

static int print_unsigned_short_recursive(unsigned short n, char *digits,
                                          int base) {
  int count = 0;

  if (n / base)
    count += print_unsigned_short_recursive(n / base, digits, base);
  count += _putchar_buffer(digits[n % base]);
  return count;
}
...
int print_unsigned_short(unsigned short n, char *digits, int base, int flags) {
  ...
  count += print_unsigned_short_recursive(n, digits, base);
  return count;
}
